...
%                            obj.ampG*exp( -0.5 * ( (y-obj.y0).^2 / obj.sigmaY^2) ) + obj.C;
        ...
        function  plotFitResults(obj, appData)  % plots the text
            ...
            text( 10, 190, ['Atoms Num: ' addCommas(obj.atomsNo)], 'fontSize', 16);
            text( 10, 145, {'fit function = A_{TF} * max(1- (x-x_0)^2/ w_x^2 - (y-y_0)^2 /w_y^2, 0)^{3/2}' ...
                '                   + A_G * {\ite}^{-(x-x_0)^2 / 2\sigma_x^2 - (y-y_0)^2 / 2\sigma_y^2} + C'}, 'fontsize', 10);
            text( 50, 100, ['A = ' num2str(obj.maxVal) ' ( A_{TF} = ' num2str(obj.ampTF) ', A_G = ' num2str(obj.ampG) ' )'], 'fontsize', 10);
            ...
            text( 50, 15, ['C = ' num2str(obj.C) ', BEC fraction = ' num2str(BECfraction) ], 'fontsize', 10);
        end
    end
end
